...
* [Fonts](/elliottneilclark/dotfiles/tree/elliott/conf/osx/fonts) via the [init/50_osx_fonts.sh](init/50_osx_fonts.sh) script
...
You might want to set up your ubuntu server [like I do it](https://github.com/elliottneilclark/dotfiles/wiki/ubuntu-setup), but then again, you might not.
...
1. Fork this repo
1. Open a terminal/shell and do this (change `elliottneilclark` and `elliott` as appropriate):

#### Ubuntu

```sh
export DOTFILES_GH_USER=elliottneilclark
export DOTFILES_GH_BRANCH=elliott
...
```sh
bash -c "$(wget -qO- https://bit.ly/elliottneilclark-dotfiles)" && source ~/.bashrc
```

#### macOS

```sh
bash -c "$(curl -fsSL https://bit.ly/elliottneilclark-dotfiles)" && source ~/.bashrc
```
